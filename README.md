...
| `oomtm.pdf` | PDF conversion. Office docs (Word/Excel/PowerPoint) use the locally-installed **Microsoft Office** via COM for best fidelity, falling back to **LibreOffice** headless; images via Pillow; `.msg`/`.eml`→HTML→PDF. Macros are force-disabled when opening external docs. `ensure_libreoffice()` auto-installs LibreOffice on the worker (winget/choco/apt, concurrency-locked) when the fallback is needed. OCR + PyMuPDF redaction land here later. Install with the `[pdf]` extra. |
